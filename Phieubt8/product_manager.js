...
function getInStock(productsList) {
  return productsList.filter(p => p.stock > 0);
}

// 2. Lọc theo category VÀ khoảng giá
function filterProducts(productsList, category, minPrice, maxPrice) {
  return productsList.filter(p => p.category === category && p.price >= minPrice && p.price <= maxPrice);
}

// 3. Sắp xếp theo giá (tăng/giảm)
function sortByPrice(productsList, order = "asc") {
  return [...productsList].sort((a, b) => (order === "asc" ? a.price - b.price : b.price - a.price));
}

// 4. Tìm sản phẩm rẻ nhất mỗi category
function cheapestByCategory(productsList) {
  return productsList.reduce((acc, p) => {
    if (!acc[p.category] || p.price < acc[p.category].price) acc[p.category] = p;
    return acc;
  }, {});
}

// 5. Tính tổng giá trị kho (price × stock cho mỗi SP)
function totalInventoryValue(productsList) {
  return productsList.reduce((sum, p) => sum + p.price * p.stock, 0);
}

// 6. Tạo mảng chỉ chứa { name, formattedPrice }
function formatProductList(productsList) {
  return productsList.map(p => ({ name: p.name, formattedPrice: p.price.toLocaleString('vi-VN') + 'đ' }));
}

// 7. Tính rating trung bình toàn bộ
function averageRating(productsList) {
  if (!productsList.length) return 0;
  return productsList.reduce((s, p) => s + p.rating, 0) / productsList.length;
}

// 8. Tìm sản phẩm theo keyword (tìm trong name, case-insensitive)
function searchProducts(productsList, keyword) {
  const k = String(keyword).toLowerCase();
  return productsList.filter(p => p.name.toLowerCase().includes(k));
}
